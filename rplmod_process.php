...
$mention = $_GET['content']; //mention
$date = $_GET['date'];  //dat
$rpl = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM comment WHERE idx = $num AND mention = '$mention' AND dat = '$date' AND id = '".$_SESSION['id']."'"));
    ...
    <div class="container">
            ...
            <div class="item">
                ...
                <div class="overscroll">
                    ...
                    <form action="rplmod_ok.php?num=<?php echo $num;?>&content='<?php echo $rpl['mention'];?>'&date='<?php echo $rpl['dat'];?>'" class="comment" method="POST">
                        <textarea name="reply" class="reply" placeholder="댓글을 입력하세요." required><?php echo $rpl['mention'];?></textarea>
                        <button name="replyBtn" class="replyBtn" type="submit">수정</button>
                    </form>
                        ...
                        if($rplist) {
                            while($row = mysqli_fetch_assoc($rplist)){
                                ...
                                if($_SESSION['id']===$row['id']){
                                    echo"<p class='rr'><a href='rplmod_process.php?num=".$num."&content=".$row['mention']."&date=".$row['dat']."'>수정 |</a><a href='rpldelete_process.php?num=".$num."&content=".$row['mention']."&date=".$row['dat']."'> 삭제</a></p>";
                                }
